...
function ItemdatabaseGetAwardCostModifiers(award)
    local res, numModifiers, modifiers
    local struct = DataView.ArrayBuffer(32*8)
    struct:SetInt32(1*8, 10)

    res          = Citizen.InvokeNative(0xE81D0378A384E755, award, struct:Buffer()) == 1
    numModifiers = struct:GetInt32(0*8)
    modifiers    = {}
    for i = 0, numModifiers - 1 do
        modifiers[i+1] = struct:GetInt32((2 + i) * 8)
    end
...
function ItemdatabaseFilloutModifier(modifier, index)
    local res, currencyType, value
    local struct = DataView.ArrayBuffer(2*8)

    res          = Citizen.InvokeNative(0x60614A0AB580A2B5, modifier, index, struct:Buffer()) == 1
    currencyType = struct:GetInt32(0*8)
    value        = struct:GetFloat32(1*8)
...
function ItemdatabaseFilloutAcquireCost(item, costType)
    local res, o0, currencyType, amount, o2
    local outData = DataView.ArrayBuffer(38*8)
    outData:SetInt32(4*8, 15)
    outData:SetInt32(36*8, 10)

    res = Citizen.InvokeNative(0x74F7928816E4E181, item, costType, outData:Buffer()) == 1
    o0 = outData:GetInt32(2*8)
    currencyType = outData:GetInt32(5*8)
    amount = outData:GetInt32(6*8)
    o2 = outData:GetInt32(37*8)
